...
all_data<-data.frame()
for(sub_idx in 1:length(subject_ids)){
  subject<-subject_ids[sub_idx]
  ...
  for(blk_idx in 1:length(blk_ends)) {
    ...
    for(i in 1:length(trial_starts)) {
      ...
      side<-log_df$checkerboard_side[log_df$overall_trial==(overall_trial-1)]
...
trial_window <- subset_by_window(data, window_start_msg = "TrialStart", 
...
onsets <- make_onset_data(response_window_clean, onset_time = 3000, 
...
df_switche <- make_switch_data(onsets, predictor_columns = "Emotion",
                                summarize_by = "Participant")

# visualize subject's switch times
plot(df_switche, predictor_columns = c("Emotion"))


model_switches <- lmer(FirstSwitch ~ Emotion + (1 | Participant), data=df_switche)
print(model_switches)

Anova(model_switches)

#Graphical comparisons

emm1 = emmeans(model_switches, specs = pairwise  ~ Emotion)
print(emm1)
...
#see the coefficients of these linear functions:
coef(pairs(emm1))
